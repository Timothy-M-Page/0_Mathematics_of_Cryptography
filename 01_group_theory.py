...
class MultiplicativeGroupMod:
    ...
    def subgroup(self, x: int) -> list[int]:
        if x not in self.group:
            raise ValueError(f'Element must be a member of the group.')
        sub = []
        exponent = x
...
class DihedralGroup:
    ...
    def product(self, x: int, y: int) -> int:
        ...
        if x not in self.group:
            raise ValueError(f'x must be an element of the group.')
        if y not in self.group:
            raise ValueError(f'y must be an element of the group.')
    ...
    def inverse(self, x: int) -> int:
        if x not in self.group:
            raise ValueError(f'Input must be an element of the group.')
        if x >= self.n:
            return x
        return (self.n - x) % self.n
...
class GeneralLinearGroupMod:
    ...
    dim with integer entries mod n. The group
    """
    def __init__(self, dim: int, mod: int):
        if not isinstance(dim, int):
            raise TypeError(f'Dimension must be a integer.')
        if not isinstance(mod, int):
            raise TypeError(f'Modulus must be an integer.')
        if mod <= 0:
            raise ValueError("Modulus must be a positive integer.")
        if dim <= 0:
...
class SymmetricGroup:
    ...
    def validate_permutation(perm: tuple) -> None:
        if not isinstance(perm, tuple):
            raise TypeError('Permutation must be a tuple of integers')
    ...
    def product(self, perm1: tuple[int], perm2: tuple[int]) -> tuple[int, ...]:
        ...
        if len(perm1) != len(perm2):
            raise ValueError(f'Permutations must be the same size.')
